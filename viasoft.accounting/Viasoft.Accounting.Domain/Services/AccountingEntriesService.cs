...
namespace Viasoft.Accounting.Domain.Services
{
    public class AccountingEntriesService : IAccountingEntriesService, ITransientDependency
    {
        ...
        public AccountingEntriesService(IRepository<AccountingEntryItem> accountingEntryItems, IRepository<AccountingEntry> accountingEntries, IMapper mapper, IUnitOfWork unitOfWork, IRepository<LegacyFechamentoPeriodoContabil> legacyFechamentoPeriodoContabils, ILogger<AccountingEntriesService> logger, IPushNotification pushNotification, ICurrentUser currentUser, IUserStore userStore, 
            IGetCodeAccountEntryService getCodeAccountEntryService, IServiceBus serviceBus, ICurrentCompany currentCompany, IDateTimeProvider dateTimeProvider, IRepository<AccountingEntryItemOrigin> accountingEntryItemOrigins)
        {
        ...
        public async Task AddAccountingEntry(AccountingEntryDto entry, List<AccountingEntryItemDto> items)
        {
            ...
            var entryItemsWithOrigins = GetEntryItemsForCreation(items, userDetails.Login, userPreferences);
            try
            {
                await InsertItemsAsync(newEntry, entryItemsWithOrigins);
            }
        ...
        private async Task InsertItemsAsync(AccountingEntry entry, List<AccountingEntryItemWithOriginsDto> entryItems)
        {
        ...
        private List<AccountingEntryItemWithOriginsDto> GetEntryItemsForCreation(List<AccountingEntryItemDto> items, string usuario, UserPreferences userPreferences)
        {
            ...
            foreach (var item in orderedItems)
            {
                ...
                foreach (var itemOrigin in item.AccountingEntryItemOrigins)
                {
                    ...
                    outputItem.Origins.Add(new AccountingEntryItemOrigin
                    {
                        ...
                        CreationDate = _dateTimeProvider.UtcNow().ToLocal(userPreferences.DefaultUserTimeZone)
                    });
                }
            }
        ...
        public async Task<MotivoEstornarLancamentoContabilResult> Estornar(Guid sourceId)
        {
            ...
            var dataLacamento = DateTime.ParseExact(lancamentoParaEstornar.EntryDateLegacy, "yyyyMMdd",
            ...
            var contemPeriodoContabilFechadoContentoLancamento = await _legacyFechamentoPeriodoContabils
                ...
                .AnyAsync(fechamento => fechamento.Data >= dataLacamento);
