...
public class CofinsRegistroF100: ICofinsDebitarRepositorio
{
    ...
    public CofinsRegistroF100(IRepository<RegistroF100> registroF100, IRepository<Fornecedor> fornecedor, IRepository<ApuracoesEntidades.LancamentoContabil> lancamentoContabil, 
    ...
    public async Task<IReadOnlyCollection<IImpostoDto>> ListarImpostoDebitar(ConciliacaoContabil conciliacaoContabil)
    {
      ...
      var registrosF100 = from r in registroF100.FiltrarPeriodoContabil(conciliacaoContabil)
            ...
            where r.IndicadorOperacao == 1 && r.ValorCofins > 0
        ...
        var registrosF100Contas =from lan in lancamentoContabil
            ...
            where f100.ParametrizacaoLancamento  && f100.IndicadorOperacao == 1     
            ...
            select new ImpostoDto
            {
                ...
                Valor = g.Key.Operacao == "D"
                    ? (g.Sum(x => x.lan.ValorDebito) * (g.Key.AliquotaCofins / 100)) * -1
                    : (g.Sum(x => x.lan.ValorCredito) * (g.Key.AliquotaCofins / 100)) * -1,
